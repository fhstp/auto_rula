/// Contains 2 element tuple (pair) utility functions.
abstract final class Pair {
  ...
  /// Selects the second/right element of a pai
  static T right<T>((T, T) pair) {
    return pair.$2;
  }

  /// Reduces the two values in the pair into a single value using the given
  /// function [f].
  ...
  static (V, V) Function((T, T), (U, U)) pairwise<T, U, V>(V Function(T, U) f) {
    return (t, u) => (
      f(Pair.left(t), Pair.left(u)),
      f(Pair.right(t), Pair.right(u)),
    );
  }

  /// Converts this pair into a 2-item [List].
  static List<T> toList<T>((T, T) pair) => [left(pair), right(pair)];
}
