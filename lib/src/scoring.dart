...
import 'package:auto_rula/src/func_ext.dart';
...
final _addPairs = Pair.pairwise((int a, int b) => a + b);

/// The result of scoring a [RulaSheet].
///
/// Contains all score values, both intermediary and full results.
@immutable
class RulaScores {
  /// Creates a [RulaScores] object from the given values.
  const RulaScores({
  ...
  factory RulaScores.calculateFor(RulaSheet sheet) {
    ...
    final upperArmScores = _addPairs(
    ...
    final armHandScore = ((
      _tableA[Pair.left(upperArmScores) - 1][Pair.left(lowerArmScores) -
          1][Pair.left(wristScores) - 1],
      _tableA[Pair.right(upperArmScores) - 1][Pair.right(lowerArmScores) -
          1][Pair.right(wristScores) - 1],
    )..pipe(Pair.map(_assertInRange(1, 9)))).pipe(Pair.reduce(worse));
    final neckTrunkLegScore =
        _tableB[neckScore - 1][trunkScore -
            1][legScores.pipe(Pair.reduce(worse)) - 1];
